#!/usr/bin/env python3
"""
Autonomous pipeline: long-running (context-aware + summarization)

Same shape as context-aware-implementation, but with progressive summarization.

DISTINCTION from context-aware-implementation:
    - Uses summarizing_invoke wrapper on top of context_aware_invoke
    - Periodically summarizes completed work into compact knowledge
    - Preserves technical decisions, file structure, and project state
    - Replaces old messages with summary + recent context
    - Use this variant for projects with 200+ commits where context would
      eventually overflow even the large model's 256k window

DISTINCTION from massive-feature-implementation:
    - Two-model fallback + progressive summarization (vs single model)
    - Use for long-running autonomous development loops
...
from four.chat_model import context_aware_invoke, summarizing_invoke
...
def main():
    parser = argparse.ArgumentParser(description="Long-running autonomous pipeline with context-aware models and progressive summarization.")
    ...
    # Chain: context_aware (two-model fallback) + summarizing (progressive compression)
    ...
    invoke = summarizing_invoke(
        base_invoke,
        summarize_model=f"openai/{LARGE_MODEL}",
        summarize_base_url=LARGE_URL,
        summarize_threshold=100_000,
        summarize_target=10_000,
        keep_recent=8,
        api_key="dummy",
    )
