...
usage() {
    ...
    echo "  VARIANT=durable-state $0 --goal 'Build something'"
    exit 0
}
...
# ── PR gating is now a V2 component (pr_gate_env) ───────────────────────────
# The agent decides semantically when a batch is complete.
# See durable-state-implementation.py for the four-function composition.
