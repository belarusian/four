#!/bin/bash
# Test Qwen3.6-27B (Q4) using Chat Completions + Tool Calls API
# Run: ./examples/test_toolcall.sh
# Run with custom endpoint: FIVE_BASE_URL=http://192.168.1.XXX:808X/v1 ./examples/test_toolcall.sh
# Output: examples/log_toolcall.txt

set -e

cd "$(dirname "$0")/.."

LOG="examples/log_toolcall.txt"
BASE_URL="${FIVE_BASE_URL:-http://192.168.1.157:8080/v1}"
PROMPT="List all .py files in the current directory, then count how many lines in the largest one. Show the final count."

echo "Running: Qwen3.6-27B + Chat Completions Tool Calls on $BASE_URL"
echo "Prompt: $PROMPT"
echo "Log: $LOG"
echo "========================================"
